...
function getFavouriteMaps() {
	...
	if ($data) {
		foreach ($data as $map) {
			$favourites->addRow([new CLink($map['label'], 'amp.php?action=map.view&sysmapid='.$map['id'])]);
		}
	}
...
//		isset($ZBX_SERVER, $ZBX_SERVER_PORT) ? $ZBX_SERVER.':'.$ZBX_SERVER_PORT : _('Zabbix server IP or port is not set!')
		isset($ZBX_SERVER, $ZBX_SERVER_PORT) ? $ZBX_SERVER.':'.$ZBX_SERVER_PORT : 'AMP服务器的IP或端口没有设置'
	]);
