 ...
 * @return CDiv
 */
function getPagingLine(&$items, $sortorder, $limitNumbers = null) {
	global $page;
	$rowsPerPage = $limitNumbers ? $limitNumbers : CWebUser::$data['rows_per_page'];	
	...
	if ($pagesCount == 1) {
		...
		$rowsComboBox = (new CComboBox('req_rows_per_page', $rowsPerPage , 'submit()'))
			->addItem(25, 25)
			->addItem(20, 20)
			->addItem(15, 15)
			->addItem(10, 10);
...
//		$table_stats = _s('Displaying %1$s to %2$s of %3$s found', $start + 1, $end, $total);
		$table_stats = '显示第 '.($start + 1).' 到第 '.$end.' 条记录, 总共'.$total.' 条记录 每页显示 ';
	}
