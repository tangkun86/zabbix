...
$widgets = [
	...
	WIDGET_FAVOURITE_MAPS => [
		...
		'links' => [
			['name' => _('Maps'), 'url' => 'amp.php?action=map.view']
		],
...
foreach ($widgets as $widgetid => $widget) {
	...
	$dashboardGrid[$col][$row] = (new CCollapsibleUiWidget($widgetid, $widget['data']))
		...
		->setHeader($widget['header'], [$icon], true, 'amp.php?action=dashboard.widget')
		->setFooter($footer);
}
...
foreach ($widgets as $widgetid => $widget) {
	...
	$widgetRefreshParams[$widgetid] = [
		'frequency' => $rate,
		'url' => 'amp.php?action='.$widget['action'],
		'counter' => 0,
		'darken' => 0,
...
<script type="text/javascript">
	...
	function addPopupValues(list) {
		...
		if (isset(list.object, favourites)) {
			...
			sendAjaxData('amp.php?action=dashboard.favourite&operation=create', {
				data: {
					object: list.object,
